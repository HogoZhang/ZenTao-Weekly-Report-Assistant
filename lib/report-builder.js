...
const EXCLUDED_DYNAMIC_LABELS = new Set(['指派了', '编辑了', '创建了']);

const EXCLUDED_DYNAMIC_CODES = new Set([
  'assigned', 'assign',
  'edited', 'edit',
  'created', 'create',
]);

const EXCLUDED_DYNAMIC_TEXT_PREFIX = /^(指派了|编辑了|创建了)(\s|$)/;

function isExcludedDynamic(dynamic) {
  if (EXCLUDED_DYNAMIC_LABELS.has(dynamic.actionLabel)) return true;

  const code = String(dynamic.actionCode || '').toLowerCase();
  if (EXCLUDED_DYNAMIC_CODES.has(code)) return true;

  const text = stripTimePrefix(dynamic.action || dynamic.text || '');
  return EXCLUDED_DYNAMIC_TEXT_PREFIX.test(text);
}
